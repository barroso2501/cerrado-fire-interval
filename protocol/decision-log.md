...
1. UNIT OF PRESENTATION: Pixel counts will be converted to area (km² or Mha) using the 30m pixel size (1 pixel = 900 m² = 0.0009 km²). No recalculation needed — direct multiplication. Key values:
...
2. NARRATIVE DISTINCTION between freq=0 and freq=1:
...
3. RESULT SELECTION confirmed: only results responding to the three objective components (structural, quantitative, ecological) enter Paper A. Spatial patterns, disruption metrics (short_tmin, long_max), MCBY by class, and ecoregion comparisons deferred to Paper B or future work.
**Data seen at time of decision:** yes
...
---
**Date:** 2026-03-24
**Decision:** Definition A correction validated — notebook 04b accepted as correct.

ROOT CAUSE OF PREVIOUS ARTEFACT:
  Notebook 04 computed uncensored intervals as (gap - 1), measuring fire-free
  years between events (Definition B). This is inconsistent with T/n, which
  implicitly measures inter-event intervals (Definition A: gap = t2 - t1).
  The verification script confirmed the discrepancy:
    freq=2:  DefB=9.699, DefA=10.699 (diff=+1.000)
    freq=5:  DefB=5.039, DefA=6.039  (diff=+1.000)
    freq=10: DefB=2.597, DefA=3.597  (diff=+0.997)
  Difference is exactly +1.000 for all classes — confirms systematic
  subtraction of 1 in notebook 04.

NOTEBOOK 04b VALIDATION:
  - All sanity checks passed
  - DefA - DefB = +1.000 (exact) for all frequency classes
  - L > U confirmed for freq=2–8; R > U confirmed for freq=2–8
  - R < U begins at freq=9 (vs freq=15 in DefB) — geometric effect confirmed
  - Censored/uncensored ratios: 2.00, 1.00, 0.67... mathematically correct
  - Continuity at freq=20→21: smooth (no artefact)

FINAL RELATIVE ERROR RESULTS (Definition A, freq=2–20):
  freq=2:  I_hat=20.000, I_obs=10.699, error=86.9%  (75,044 km²)
  freq=3:  I_hat=13.333, I_obs=8.460,  error=57.6%  (52,583 km²)
  freq=4:  I_hat=10.000, I_obs=7.011,  error=42.6%  (39,167 km²)
  freq=5:  I_hat=8.000,  I_obs=6.039,  error=32.5%  (30,611 km²)
  freq=6:  I_hat=6.667,  I_obs=5.311,  error=25.5%  (24,682 km²)
  freq=7:  I_hat=5.714,  I_obs=4.739,  error=20.6%  (20,356 km²)
  freq=8:  I_hat=5.000,  I_obs=4.283,  error=16.7%  (17,184 km²)
  freq=10: I_hat=4.000,  I_obs=3.596,  error=11.2%  (12,832 km²)
  freq=15: I_hat=2.667,  I_obs=2.551,  error=4.6%   (6,258 km²)
  freq=20: I_hat=2.000,  I_obs=1.952,  error=2.4%   (3,046 km²)

KEY OBSERVATION:
  Error exceeds 50% only at freq=2 (87%). Error exceeds 25% at freq=2–5.
  The 50% threshold is crossed between freq=2 and freq=3.
  The 25% threshold is crossed between freq=5 and freq=6.
  Curve is monotonically decreasing throughout — no inflection artefact.
  Previous inflection (freq=8–10) was entirely due to Definition B error.

AREA IN CONTEXT:
  All pixels with freq >= 2 cover 37.44% of stable native vegetation mask.
  Pixels with error > 50% (freq=2 only): 75,044 km² (8.0% of mask).
  Pixels with error > 25% (freq=2–5):   197,405 km² (21.1% of mask).
  Pixels with error < 10% (freq>=11):    75,358 km² (8.1% of mask).

FILES:
  classification/part_b_summary_defA.csv
  classification/part_b_interval_distributions_defA.csv
  classification/relative_error_by_freq_v3.csv
  figures/fig_relative_error_v3.pdf / .png
  scripts: 04b_integrated_distributions_defA.py, 07_relative_error_v3.py

PENDING:
  - Figure annotation: show area in km² instead of Mha (values round to 0.0)
  - Update results section 3.3 with final values
  - Update part_b_summary used in figures 2-4 (notebooks 05, 06) to defA

**Status:** POST-HOC — all decisions made after seeing data and figures
